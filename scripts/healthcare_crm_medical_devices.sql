...
CREATE TABLE `medical_devices` (
  ...
  `assigned_to_patient_id` int DEFAULT NULL,
  `assigned_date` date DEFAULT NULL,
  `returned_date` date DEFAULT NULL,
  PRIMARY KEY (`device_id`),
  UNIQUE KEY `serial_number` (`serial_number`),
  KEY `idx_device_patient` (`assigned_to_patient_id`),
  KEY `idx_device_serial` (`serial_number`),
  CONSTRAINT `medical_devices_ibfk_1` FOREIGN KEY (`assigned_to_patient_id`) REFERENCES `patients` (`patient_id`) ON DELETE SET NULL
...
LOCK TABLES `medical_devices` WRITE;
